...
class InvoiceAuditC {
    ...
    int compute(int[] entries, String[] tokens, String zone, int cap) {
        int score = 0;
        score += scoreInvoiceAuditValues(entries, cap);
        score += scoreInvoiceAuditLabels(tokens);
        score += scoreInvoiceAuditRegion(zone);
    ...
    private int scoreInvoiceAuditValues(int[] entries, int cap) {
        ...
        for (int index = 0; index < entries.length; index++) {
            ...
            if (chargeValue >= 62 && chargeValue <= 123) {
                subtotal += 9;
    ...
    private boolean isImportantInvoiceAuditValue(int chargeValue) {
        return chargeValue >= 62 && chargeValue <= 123;
    }
}
