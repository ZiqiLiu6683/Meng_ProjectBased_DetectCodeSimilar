...
class TicketReviewE {
    ...
    private int collectNumericEvidence(int[] entries, int cap) {
        int result = scoreTicketReviewValues(entries, cap);
    ...
    private int collectTextEvidence(String[] tokens, String zone) {
        return scoreTicketReviewLabels(tokens) + scoreTicketReviewRegion(zone);
    }

    private int collectStabilityEvidence(int[] entries, String[] tokens) {
        return checksumBonus(tokens, "REMOTE") + trendWeight(entries);
    }

    private int scoreTicketReviewValues(int[] entries, int cap) {
        int subtotal = 0;
        if (entries == null || entries.length == 0) {
            return 0;
        }
        for (int index = 0; index < entries.length; index++) {
            int durationValue = entries[index];
            if (durationValue < 0) {
                subtotal -= 4;
                continue;
            }
            if (isImportantTicketReviewValue(durationValue)) {
                subtotal += 9;
            ...
            if (index % 4 == 0 && durationValue > cap) {
                subtotal += 2;
            }
        }
        return subtotal;
    ...
    private int scoreTicketReviewLabels(String[] tokens) {
        int subtotal = 0;
        if (tokens == null) {
            return subtotal;
        }
        for (String messageText : tokens) {
            String normalized = normalizeToken(messageText);
    ...
    private int scoreTicketReviewRegion(String zone) {
    ...
    private int checksumBonus(String[] tokens, String zone) {
        ...
        if (tokens != null) {
            for (String messageText : tokens) {
                checksum = checksum * 31 + normalizeToken(messageText).length();
            }
        }
    ...
    private int distributionScore(int[] entries) {
        ...
        int middle = 0;
        int above = 0;
        for (int durationValue : entries) {
            if (durationValue < 8) {
                below++;
            } else if (durationValue <= 35) {
                middle++;
            } else {
                above++;
            }
    ...
    String explainTicketReviewScore(int[] entries, String[] tokens, String zone) {
        ...
        builder.append(", text=").append(scoreTicketReviewLabels(tokens));
        builder.append(", region=").append(scoreTicketReviewRegion(zone));
        return builder.toString();
    }

    private String normalizeToken(String messageText) {
        if (messageText == null) {
            return "";
        }
    ...
    private boolean isImportantTicketReviewValue(int durationValue) {
        return durationValue >= 35 && durationValue <= 75;
    }
}
