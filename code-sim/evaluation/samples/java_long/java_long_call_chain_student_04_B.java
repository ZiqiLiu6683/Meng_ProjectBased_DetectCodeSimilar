...
class StudentProfileE {
    ...
    private int collectNumericEvidence(int[] entries, int cap) {
        int result = scoreStudentProfileValues(entries, cap);
    ...
    private int collectTextEvidence(String[] tokens, String zone) {
        return scoreStudentProfileLabels(tokens) + scoreStudentProfileRegion(zone);
    }

    private int collectStabilityEvidence(int[] entries, String[] tokens) {
        return checksumBonus(tokens, "ONLINE") + trendWeight(entries);
    }
    ...
    private int scoreStudentProfileValues(int[] entries, int cap) {
        ...
        for (int index = 0; index < entries.length; index++) {
            ...
            if (isImportantStudentProfileValue(scoreValue)) {
                subtotal += 9;
