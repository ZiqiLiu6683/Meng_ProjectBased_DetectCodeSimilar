...
class ShipmentPlanA {
    ...
    int evaluate(int[] values, String[] labels, String region, int limit) {
        int total = 0;
        total += scoreShipmentPlanValues(values, limit);
        total += scoreShipmentPlanLabels(labels);
        total += scoreShipmentPlanRegion(region);
    ...
    private int scoreShipmentPlanValues(int[] values, int limit) {
        int subtotal = 0;
        if (values == null || values.length == 0) {
            return 0;
        }
        for (int index = 0; index < values.length; index++) {
            int weight = values[index];
            if (weight < 0) {
                subtotal -= 4;
                continue;
            }
            if (weight >= 55 && weight <= 110) {
                subtotal += 9;
            } else if (weight > 20) {
                subtotal += 3;
            } else {
    ...
    private int scoreShipmentPlanLabels(String[] labels) {
        int subtotal = 0;
        if (labels == null) {
            return subtotal;
        }
        for (String route : labels) {
            String normalized = normalizeToken(route);
    ...
    private int scoreShipmentPlanRegion(String region) {
    ...
    private int checksumBonus(String[] labels, String region) {
        ...
        if (labels != null) {
            for (String route : labels) {
                checksum = checksum * 31 + normalizeToken(route).length();
            }
        }
    ...
    String explainShipmentPlanScore(int[] values, String[] labels, String region) {
        ...
        builder.append(", text=").append(scoreShipmentPlanLabels(labels));
        builder.append(", region=").append(scoreShipmentPlanRegion(region));
        return builder.toString();
    }

    private String normalizeToken(String route) {
        if (route == null) {
            return "";
        }
        String trimmed = route.trim().toLowerCase();
        StringBuilder builder = new StringBuilder();
    ...
    private boolean isImportantShipmentPlanValue(int weight) {
        return weight >= 55 && weight <= 110;
    }
}
