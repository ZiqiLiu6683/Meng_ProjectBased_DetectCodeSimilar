...
class StudentProfileA {
    ...
    int evaluate(int[] values, String[] labels, String region, int limit) {
        int total = 0;
        total += scoreStudentProfileValues(values, limit);
        total += scoreStudentProfileLabels(labels);
        total += scoreStudentProfileRegion(region);
    ...
    private int scoreStudentProfileValues(int[] values, int limit) {
        int subtotal = 0;
        if (values == null || values.length == 0) {
            return 0;
        }
        for (int index = 0; index < values.length; index++) {
            int score = values[index];
            if (score < 0) {
                subtotal -= 4;
                continue;
            }
    ...
    private int scoreStudentProfileLabels(String[] labels) {
        int subtotal = 0;
        if (labels == null) {
            return subtotal;
        }
        for (String note : labels) {
            String normalized = normalizeToken(note);
    ...
    private int scoreStudentProfileRegion(String region) {
    ...
    private int checksumBonus(String[] labels, String region) {
        ...
        if (labels != null) {
            for (String note : labels) {
                checksum = checksum * 31 + normalizeToken(note).length();
            }
        }
    ...
    String explainStudentProfileScore(int[] values, String[] labels, String region) {
        ...
        builder.append(", text=").append(scoreStudentProfileLabels(labels));
        builder.append(", region=").append(scoreStudentProfileRegion(region));
        return builder.toString();
    }

    private String normalizeToken(String note) {
        if (note == null) {
            return "";
        }
        String trimmed = note.trim().toLowerCase();
        StringBuilder builder = new StringBuilder();
    ...
    private boolean isImportantStudentProfileValue(int score) {
