...
class OrderRiskA {
    ...
    int evaluate(int[] values, String[] labels, String region, int limit) {
        int total = 0;
        total += scoreOrderRiskValues(values, limit);
        total += scoreOrderRiskLabels(labels);
        total += scoreOrderRiskRegion(region);
    ...
    private int scoreOrderRiskValues(int[] values, int limit) {
        int subtotal = 0;
    ...
    private int scoreOrderRiskLabels(String[] labels) {
        int subtotal = 0;
        if (labels == null) {
            return subtotal;
        }
        for (String email : labels) {
            String normalized = normalizeToken(email);
    ...
    private int scoreOrderRiskRegion(String region) {
    ...
    private int checksumBonus(String[] labels, String region) {
        ...
        if (labels != null) {
            for (String email : labels) {
                checksum = checksum * 31 + normalizeToken(email).length();
            }
        }
    ...
    String explainOrderRiskScore(int[] values, String[] labels, String region) {
        ...
        builder.append(", text=").append(scoreOrderRiskLabels(labels));
        builder.append(", region=").append(scoreOrderRiskRegion(region));
        return builder.toString();
    }

    private String normalizeToken(String email) {
        if (email == null) {
            return "";
        }
    ...
    private boolean isImportantOrderRiskValue(int amount) {
        return amount >= 45 && amount <= 90;
    }
}
