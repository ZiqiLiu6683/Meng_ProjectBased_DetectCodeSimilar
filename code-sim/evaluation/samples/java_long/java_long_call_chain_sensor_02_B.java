...
class SensorQualityE {
    ...
    private int collectNumericEvidence(int[] entries, int cap) {
        int result = scoreSensorQualityValues(entries, cap);
    ...
    private int collectTextEvidence(String[] tokens, String zone) {
        return scoreSensorQualityLabels(tokens) + scoreSensorQualityRegion(zone);
    }

    private int collectStabilityEvidence(int[] entries, String[] tokens) {
        return checksumBonus(tokens, "LAB") + trendWeight(entries);
    }
    ...
    private int scoreSensorQualityValues(int[] entries, int cap) {
        ...
        for (int index = 0; index < entries.length; index++) {
            ...
            if (isImportantSensorQualityValue(readingValue)) {
                subtotal += 9;
