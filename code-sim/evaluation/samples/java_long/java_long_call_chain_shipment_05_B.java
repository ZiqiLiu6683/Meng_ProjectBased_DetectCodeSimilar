...
class ShipmentPlanE {
    ...
    private int collectNumericEvidence(int[] entries, int cap) {
        int result = scoreShipmentPlanValues(entries, cap);
    ...
    private int collectTextEvidence(String[] tokens, String zone) {
        return scoreShipmentPlanLabels(tokens) + scoreShipmentPlanRegion(zone);
    }

    private int collectStabilityEvidence(int[] entries, String[] tokens) {
        return checksumBonus(tokens, "AIR") + trendWeight(entries);
    }
