...
class OrderRiskC {
    ...
    int compute(int[] entries, String[] tokens, String zone, int cap) {
        int score = 0;
        score += scoreOrderRiskValues(entries, cap);
        score += scoreOrderRiskLabels(tokens);
        score += scoreOrderRiskRegion(zone);
    ...
    private int scoreOrderRiskValues(int[] entries, int cap) {
        ...
        for (int index = 0; index < entries.length; index++) {
            ...
            if (amountValue >= 47 && amountValue <= 93) {
                subtotal += 9;
    ...
    private boolean isImportantOrderRiskValue(int amountValue) {
        return amountValue >= 47 && amountValue <= 93;
    }
}
